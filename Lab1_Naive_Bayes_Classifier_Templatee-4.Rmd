...
t_counts <- tidy_text %>% count(splitted,sort=TRUE)
tidy_text
```
...
```{r}
rtidy_text <- unnest_tokens(train, 'splitted', 'text', token="words") %>%
             filter(splitted %in% splitted_stop_words)

r_counts <- rtidy_text %>% count(splitted,sort=TRUE)
top_r <- head(r_counts, 20)
top_t <- head(t_counts, 20)
# Convert table() to data frame for ggplot
p1 <- ggplot(top_r, aes(x=reorder(splitted, n), y=n)) +
  geom_col(fill="steelblue") +
  coord_flip() +
  labs(title="Top 20 stop words", x=NULL, y="Count")

p2 <- ggplot(top_t, aes(x=reorder(splitted, n), y=n)) +
  geom_col(fill="darkorange") +
  coord_flip() +
  labs(title="Top 20 clean words", x=NULL, y="Count")

# Base R plots for all words
rtidy_table <- as.data.frame(table(rtidy_text$splitted)) %>% arrange(Var1)
tidy_table <- as.data.frame(table(tidy_text$splitted)) %>% arrange(Var1)


# p1 and p2: print ggplots into the base R window using print()
print(p1)
print(p2)

# p3 and p4: base R barplots, alphabetical, without labels
barplot(rtidy_table$Freq, names.arg = NA, main="All stop words", col="firebrick")
barplot(tidy_table$Freq, names.arg = NA, main="All clean words", col="forestgreen")
